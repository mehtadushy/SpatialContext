function [] = ComputeScaleWeightedRelativeOrientationCorrectedFrame(imageFileList, dataBaseDir, params, canSkip)
...
%                  relative_weight = relative_weight/norm(relative_weight);
                  ...
                  %Weigh by scale of the neighbourhood keypoints
		  sqrt_scale = sqrt(relevant_frame(3,:));
                  relative_weight =sqrt_scale / sum(sqrt_scale);% ones(size(relative_orientations))/length(relative_orientations);
