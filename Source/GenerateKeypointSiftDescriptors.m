function [ ] = GenerateKeypointSiftDescriptors(imageFileList, imageBaseDir, dataBaseDir, params, canSkip, pfig)
    if(~exist('params','var'))
...
%%
    for f = 1:length(imageFileList)
        ...
        outFName = fullfile(dataBaseDir, sprintf('%s_keypoint_sift.mat', baseFName));
        ...
        if(~params.axisAlignedSIFT)
            ...
            [frame, desc] = vl_sift(single(I), 'magnif', params.SIFTMagnif);
        else
            frame = vl_sift(single(I));
            [frame, desc] = vl_sift(single(I), 'magnif', params.SIFTMagnif, 'frames', [frame(1:3,:); zeros(1,size(frame,2))]);
        end
        features.wid = wid;
        features.hgt = hgt;
