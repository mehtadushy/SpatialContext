...
parfor k = 1:length(dictionarySize)
 for j = 1:length(coarseDictionarySize)
    ...
    BuildSoftAssignVLAD(data.filenames,image_dir,data_dir,sprintf('_%d_neigh_contexthist%d.mat', neighbourhoodSize(i), coarseDictionarySize(j)),para, 0, pfig);
    close(pfig)
    end
 end
end
...
parfor j = 1: length(keypointDictionarySize)
    for l = 1: length(coarseDictionarySize)
     for k = 1: length(contextDictionarySize)
      for i = 1:length(neighbourhoodSize)
            pfig = sp_progress_bar(sprintf('3D Hist of Neigh Size%d Keypt Dic%d Context Dic%d',neighbourhoodSize(i), keypointDictionarySize(j), contextDictionarySize(k)));
            CompileVLADContext3DSoftAssignHistogram( data.filenames,data_dir, sprintf('_soft_texton_ind_%d_%d_neigh_ROcontext_keypoint_sift',keypointDictionarySize(j),neighbourhoodSize(i)), sprintf('_soft_texton_ind_%d_%d_neigh_contexthist%d', contextDictionarySize(k), neighbourhoodSize(i), coarseDictionarySize(l)), [keypointDictionarySize(j), contextDictionarySize(k), coarseDictionarySize(l)],[] , 0, pfig );
            close(pfig)
      end
    end
    end
