function [ ] = GenerateDoGKeypointSiftDescriptors(imageFileList, imageBaseDir, dataBaseDir, params, canSkip, pfig)
    if(~exist('params','var'))
...
%%
    for f = 1:length(imageFileList)
        ...
        if(~params.axisAlignedSIFT)
            ...
            [frame, desc] = vl_sift(single(I), 'magnif', params.SIFTMagnif, 'frames', frame(1:4,:), 'orientations');
        else
            frame = vl_covdet(single(I));
            [frame, desc] = vl_sift(single(I), 'magnif', params.SIFTMagnif, 'frames', frame(1:4,:));
        end
        features.wid = wid;
        features.hgt = hgt;
