function [ H_all ] = BuildSoftAssignVLAD( imageFileList,imageBaseDir, dataBaseDir, featureSuffix, params, canSkip, pfig )
...
for f = 1:length(imageFileList)
    ...
    texton_ind.vlad = zeros(ndata,3, size(features.data,2));
    ...
    if ndata <= batchSize
        ...
        texton_ind.vlad = arrayfun(@(x) repmat(reshape(features.data(x,:),[1,1, size(features.data,2)]),1,3,1), (1:ndata)', 'un',0);
        texton_ind.vlad = cell2mat(texton_ind.vlad);
        texton_ind.vlad = texton_ind.vlad - cell2mat(arrayfun(@(x) [reshape(dictionary(min_ind(x,1),:),[1,1, size(features.data,2)]), reshape(dictionary(min_ind(x,2),:),[1,1, size(features.data,2)]), reshape(dictionary(min_ind(x,3),:),[1,1, size(features.data,2)])], (1:ndata)', 'un', 0));        
        ...
        for j = 1:batchSize:ndata
            ...
            texton_ind.vlad(lo:hi, :) = cell2mat(arrayfun(@(x) repmat(reshape(features.data(x,:),[1,1, size(features.data,2)]),1,3,1), (lo:hi)', 'un',0));
            texton_ind.vlad(lo:hi, :) = texton_ind.vlad(lo:hi, :) - cell2mat(arrayfun(@(x) [reshape(dictionary(texton_ind.data(x,1),:),[1,1, size(features.data,2)]), reshape(dictionary(texton_ind.data(x,2),:),[1,1, size(features.data,2)]), reshape(dictionary(texton_ind.data(x,3),:),[1,1, size(features.data,2)])], (lo:hi)', 'un',0));
