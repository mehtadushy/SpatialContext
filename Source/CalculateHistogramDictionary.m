function [ dictionary ] = CalculateHistogramDictionary( imageFileList, dataBaseDir, featureSuffix, params, canSkip, pfig )
%function [ ] = CalculateDictionary( imageFileList, dataBaseDir, featureSuffix, params, canSkip )
%
%Create the keypoint neighbourhood histogram dictionary (dictionary of
%dense SIFT histograms
%
% First, all of the neighbourhood histogram descriptors are loaded for a random set of images. The
...
%  Its default value is '_6_10_neighbourhood_histogram_50.mat'.
...
fprintf('Building Histogram Clustering Dictionary\n\n');

%% parameters

reduce_flag = 1;
%ndata_max = 100000; %use 4% avalible memory if its greater than the default
ndata_max = 50000; %use 4% avalible memory if its greater than the default
...
outFName = fullfile(dataBaseDir, sprintf('dictionary_%d%s', params.dictionarySize, featureSuffix));
...
for f = 1:params.numTextonImages    
    ...
    desc_all = [desc_all; data2add];
    %fprintf('Loaded %s, %d descriptors, %d so far\n', inFName, ndata, size(desc_all,1));
...
centers = zeros(params.dictionarySize, size(desc_all,2));

%% run kmeans
%fprintf('\nRunning k-means. We should be running kmedoids with intersectionDistance\n %d NaNs in Data!!!!!\n',sum(sum(isnan(desc_all))));
fprintf('\nRunning k-medoids with intersection distance\n');
%dictionary = sp_kmeans(centers, desc_all, options); %this is not the way to do it
dictionary = kmedoids_custom(centers, desc_all, options, @intersectionDistance);
    
fprintf('Saving neighborhood pooling histogram dictionary\n');
