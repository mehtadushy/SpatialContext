function [] = ComputeNeighborhoodPoolingSoftAssignHistogram( imageFileList, dataBaseDir, softTextonSuffix, params, canSkip)
    %This compiles histograms of soft-assigned dense SIFT descriptors in
    %the neighbourhood of keypoint
    if(~exist('params','var'))
...
%%
    ...
    for f = 1:length(imageFileList)
        ...
        features.neighSize = zeros(length(features.x));
        %Load dense sift texton indices
        in_fname = fullfile(dataBaseDir, sprintf('%s%s', baseFName, softTextonSuffix));
        ...
        for sf = 1:size(params.sigmaFactors,1)
            ...
            for k = 1:length(features.x)
                ...
                for s = 1:2
                   scale = sigmaFactor(s);
                   neighbourhoodMask = zeros(size(texton_ind.x));
                   x = features.x(k);
                   y = features.y(k);
                   scale = scale * keypt_features.vl_frame(3,k);
                    ...
                    texton_weights = texton_ind.weights(neighbourhoodMask == 1, :);
                    sf_size(s) = size(texton_region,1);
                    for d_idx = 1:sf_size(s) 
                        sf_hist(s,texton_region(d_idx,1)) =  sf_hist(s,texton_region(d_idx,1)) + texton_weights(d_idx,1);
                        sf_hist(s,texton_region(d_idx,2)) =  sf_hist(s,texton_region(d_idx,2)) + texton_weights(d_idx,2);
                        sf_hist(s,texton_region(d_idx,3)) =  sf_hist(s,texton_region(d_idx,3)) + texton_weights(d_idx,3);

                    end
                end
                
                features.data(k,:) = (sf_hist(2,:) - sf_hist(1,:));
                features.neighSize(k) = sf_size(2) - sf_size(1);
