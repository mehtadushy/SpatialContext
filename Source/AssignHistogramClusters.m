function [ H_all ] = AssignHistogramClusters( imageFileList, dataBaseDir, featureSuffix, params, canSkip, pfig )
...
for f = 1:length(imageFileList)
    ...
    outFName = fullfile(dataBaseDir, sprintf('%s_texton_ind_%d%s', baseFName, params.dictionarySize, featureSuffix));
    outFName2 = fullfile(dataBaseDir, sprintf('%s_hist_%d%s', baseFName, params.dictionarySize, featureSuffix));
    ...
    batchSize = 100000;
    if ndata <= batchSize
        dist_mat = intersectionDistance(features.data, dictionary);
        ...
        for j = 1:batchSize:ndata
            ...
            dist_mat = intersectionDistance(features.data(lo:hi,:), dictionary);
            [min_dist, min_ind] = min(dist_mat, [], 2);
    ...
    save(outFName2, 'H');  %it would be fun to see how well these act as independent features
end

%% save histograms of all images in this directory in a single file
outFName = fullfile(dataBaseDir, sprintf('histograms_%d%s', params.dictionarySize, featureSuffix));
