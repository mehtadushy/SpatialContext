function [centres, options, post, errlog] = kmedoids_custom(centres, data, options, distance_function)
% KMEANS	Trains a k medoid cluster model for histograms

% Adapted from Netlab neural network software:
% http://www.ncrg.aston.ac.uk/netlab/index.php
%
%	Description
%	 CENTRES = KMEDOIDS_CUSTIM(CENTRES, DATA, OPTIONS, DISTANCE_FUNCTION)
%	 uses the batch K-medoids algorithm to set the centres of a cluster model. The matrix DATA
%	represents the data which is being clustered, with each row
%	corresponding to a vector.
%	The point at which a local minimum is achieved is returned as
%	CENTRES.  The error value at that point is returned in OPTIONS(8).
%
%	[CENTRES, OPTIONS, POST, ERRLOG] = KMEDOIDS_CUSTOM(CENTRES, DATA, OPTIONS, DISTANCE_FUNCTION)
...
%
%	Copyright (c) Dushyant Mehta (2015-)
...
%Check if a distance function was passed
if(~exist('distance_function','var'))
    distance_function = @sp_dist2;
end
...
for n = 1:niters
  ...
  d2 = distance_function(data, centres);
  % Assign each point to nearest centre
  [minvals, index] = min(d2', [], 1);
  post = id(index,:);

  num_points = sum(post, 1); %number of points in each cluster
  % Adjust the centres based on new posteriors
  for j = 1:ncentres
    if (num_points(j) > 0)
      %Find the point with the shortest combined distance to all points
      %within the cluster
      cluster_data = data(post(:,j)==1, :);
      d2 = distance_function(cluster_data, cluster_data);
      cluster_dist = sum(d2,1);
      [~, center_idx]  = min(cluster_dist, [], 2);
      centres(j,:) = cluster_data(center_idx,:);
    end
  end

  % Error value is sum of (positive/squared) distance from cluster centres
  e = sum(minvals);
  if store
    errlog(n) = e;
  end
