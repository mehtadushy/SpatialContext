function [ ] = GenerateKeypointSiftFromFrames(imageFileList, imageBaseDir, dataBaseDir, params, canSkip, pfig)
    ...
    if(~isfield(params,'SIFTMagnifCoarse'))
        params.SIFTMagnifCoarse = 3;  
    end
...
%%
    for f = 1:length(imageFileList)
        ...
        outFName2 = fullfile(dataBaseDir, sprintf('%s%s_keypoint_sift_context.mat', baseFName, params.keypointFramePrefix));
        ...
        [frame, desc] = vl_sift(single(I), 'magnif', params.SIFTMagnifCoarse, 'frames', frame);
        features.data = sp_normalize_sift(double(desc'));
        %features.vl_frame = frame;
        sp_make_dir(outFName2);
